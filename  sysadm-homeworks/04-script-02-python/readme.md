...
| Вопрос  | Ответ                                                                                              |
| ------------- |----------------------------------------------------------------------------------------------------|
| Какое значение будет присвоено переменной `c`?  | Скрипт не выполнится, т.к. пытаемся сложить число и строку.                                        |
| Как получить для переменной `c` значение 12?  | Чтобы получить значение 12, необходимо преобразовать переменную 'a' к строковому виду: a = str(a). |
| Как получить для переменной `c` значение 3?  | Чтобы получить значение 3, необходимо преобразовать переменную 'b' к числовому виду: b=int(b).     |
...
bash_command = ["cd ~/py-test", "git status"]
result_os_abs_dir = os.popen('pwd').read()
result_os = os.popen(' && '.join(bash_command)).read()
is_change = False
for result in result_os.split('\n'):
    if result.find('modified') != -1:
        prepare_result = result.replace('\tmodified:   ', '')
        print(f'{result_os_abs_dir} {prepare_result}\n')
```
...
bash_command = ['cd {}'.format(sys.argv[1]), 'git status']
result_os_abs_dir = os.popen('pwd').read()
result_os = os.popen(' && '.join(bash_command)).read()
is_change = False
for result in result_os.split('\n'):
...
while True:
    for host in hosts:
        host_ip_addresses = socket.gethostbyname(host)
        if host_ip_addresses == history[host]:
            print(f'{host} - {host_ip_addresses}')
        else:
            print(f'[ERROR] {host} IP mismatch: {history[host]} {host_ip_addresses}')
            history[host]=host_ip_addresses
